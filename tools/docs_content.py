...
def build(fig, IMAGE, REPO):
  ...
  {IMAGE}:latest</pre>
    ...
    <div class="prose">
      ...
      <p>When you connect, the device lists every YANG module it implements,
      along with the revision it uses, the optional features it has turned on,
      and any vendor deviations it applies. You can read that list over either
      protocol — NETCONF, or RESTCONF for devices that do not run NETCONF at
      all — and choose which of those modules to download. The download runs in the background, so you
...
"""))
  ...
  &amp;deviations=<span class="warn">cisco-xe-ietf-ip-deviation</span></pre>
      ...
      <h2 id="transport">NETCONF or RESTCONF</h2>
      <p>The chooser next to Connect decides which protocol is used to find
      the device's models and fetch them. It is worth understanding what the
      two actually do differently, because for most devices the answer that
      comes back is the same either way.</p>

      <p>NETCONF reads the capability list out of the <code>&lt;hello&gt;</code>
      the device sends when the session opens, and then asks for each module
      with <code>&lt;get-schema&gt;</code>. RESTCONF reads the same information
      from the device's YANG library over HTTPS, and downloads each module
      from the URL the library gives for it. No session is involved.</p>

      <p>Run both against the same router and you can see how closely they
      agree. These are the real numbers from the IOS-XE device used throughout
      these pages:</p>

      <div class="scroll"><table>
        <thead><tr><th></th><th>NETCONF</th><th>RESTCONF</th></tr></thead>
        <tbody>
          <tr><td>Modules advertised</td><td>507</td><td>507</td></tr>
          <tr><td>Revisions that disagree</td><td colspan="2">none</td></tr>
          <tr><td>Modules carrying deviations</td><td>26</td><td>26, identical</td></tr>
          <tr><td>Only on this side</td>
              <td><code>ietf-netconf</code>, <code>ietf-netconf-with-defaults</code></td>
              <td><code>ietf-restconf</code>, <code>ietf-yang-patch</code></td></tr>
        </tbody>
      </table></div>

      <p>The only difference is that each protocol advertises its own
      modules, which is exactly what you would want. Everything downstream —
      the repository, the set, the tree — is identical whichever one you
      picked.</p>

      <h3>When to choose RESTCONF</h3>
      <ul>
        <li><b>The device does not run NETCONF.</b> Plenty of lab and sandbox
          devices enable only RESTCONF, and without this there would be no way
          to get their models at all.</li>
        <li><b>You are downloading a lot of modules.</b> A NETCONF download is
          hundreds of requests down one session, and if that session drops
          part-way the requests after it fail too. Over RESTCONF each module is
          an independent request, so one failure says nothing about the next.</li>
      </ul>

      <h3>When RESTCONF will not work</h3>
      <p>RESTCONF discovery needs two things from the device: the
      <code>ietf-yang-library</code> module, and a download URL for each module
      listed in it. Most platforms provide both, but listing a module and being
      able to fetch its source are separate questions, and there are two ways
      the second one goes wrong.</p>

      <p><b>The library has no download URLs.</b> Some devices publish entries
      carrying nothing but a name, a namespace and a revision. The module list
      is still correct and still worth having — it pins revisions, features and
      deviations — but there is nothing to download from. YANG Studio shows a
      <span class="tag">downloadable</span> count next to the module count when
      the two differ, so you know before you start.</p>

      <p><b>The URLs are published but not served.</b> A device behind a
      gateway often advertises URLs pointing at its own inside address, on a
      path the gateway does not proxy. Every fetch comes back 404 even though
      the module list was perfect. YANG Studio stops after a few of those and
      says what happened rather than working through the whole list.</p>

      <div class="note">
        <p>There are two versions of the library and devices are split between
        them, so both are tried. A device can answer both and put the download
        URLs in only one, so the choice is made on which library is actually
        usable rather than which replied first. The badge above the module list
        tells you which one was used.</p>
      </div>

      <h3>If the source cannot be downloaded at all</h3>
      <p>You are not stuck. The module list on its own is the valuable half —
      it tells you exactly which revisions the device implements, which
      features it has turned on, and which deviations it applies. Get the
      model files from elsewhere, such as the vendor's public repository, and
      upload them into a repository. Then build the set from the device as
      normal: the files come from the repository and the revisions, features
      and deviations come from the device, so the set is still pinned to that
      box.</p>

      <h2 id="download">What you are downloading</h2>
      <p>When you press Download, YANG Studio makes one request per module
      you picked — a <code>&lt;get-schema&gt;</code> over NETCONF, or a plain
      GET over RESTCONF — and the device replies with the module's source
      text. That text <em>is</em>
...
"""))
    ...
    <div class="prose">
      ...
      <div class="note">
        ...
        <p>If the device also has <code>ip http server</code> enabled, the
        plain HTTP port stays open and answers requests, but it returns
        <code>404 Not Found</code> for every RESTCONF URL. The web server is
        running; the RESTCONF endpoint is simply not attached to it. A 404
        over <code>http://</code> for a URL that works over
        <code>https://</code> is almost always this rather than a wrong
        path.</p>
      </div>
