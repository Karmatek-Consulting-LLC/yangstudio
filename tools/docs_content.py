...
"""))
  ...
  &amp;deviations=<span class="warn">cisco-xe-ietf-ip-deviation</span></pre>
      ...
      <p>RESTCONF discovery needs the device to implement
      <code>ietf-yang-library</code> and to publish a download URL for each
      module in it. Most modern platforms do both. A device that publishes the
      library but leaves the URL empty can still list its modules — the count
      will be right — but the download will report that it has nowhere to
      fetch them from, and NETCONF is the way to get them.</p>

      <div class="note">
        <p>There are two versions of the library and devices are split between
        them, so YANG Studio asks for the newer one (RFC 8525) and falls back
        to the older (RFC 7895). The badge above the module list tells you
        which one answered.</p>
      </div>
